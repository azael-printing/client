...
function ProformaSheet({ docNumber, customerName, tin, rows, totals }) {
  const rowCount = Math.max(1, rows.length);
  const maxBodyHeight = 16.2;
  const rowH = Math.min(4.8, maxBodyHeight / rowCount);
  ...
  return (
    <div className="sheet-page relative overflow-hidden bg-white text-[#1f2937]">
      ...
      <div className="absolute right-[8.4%] top-[17.2%] text-right text-[1.9%] leading-[1.45] font-semibold text-[#4169b2]">
        <div>
          <span className="font-bold">TIN</span>{" "}
          <span className="text-[#111111] font-medium">0082555133</span>
        </div>
        <div>
      ...
      <div className="absolute left-[38.2%] top-[29.25%] max-w-[36%] overflow-hidden whitespace-nowrap text-ellipsis bg-transparent px-[2px] text-[1.85%] text-[#111111] font-semibold text-center">
        {customerName}
      </div>

      <div className="absolute left-[38.2%] top-[32.7%] max-w-[36%] overflow-hidden whitespace-nowrap text-ellipsis bg-transparent px-[2px] text-[1.85%] text-[#111111] font-semibold text-center">
        {tin}
      </div>

      <div className="absolute left-[3.65%] top-[38.2%] w-[92%] text-[1.55%] text-[#1679bf] font-semibold">
        <div
          className="grid grid-cols-[4.5%_46.5%_19%_18%_12%] text-transparent"
          style={{ height: "4.2%" }}
        >
          <div />
          <div />
        ...
        {Array.from({ length: rowCount }).map((_, idx) => {
          ...
          return (
            <div
              key={row.id || idx}
              className="grid grid-cols-[4.5%_46.5%_19%_18%_12%]"
              style={{ height: `${rowH}%` }}
            >
              <div className="flex items-center justify-center text-[1.75%] text-[#111111] font-semibold">
                {row.no}
              </div>
              <div className="flex items-center px-[2.2%] text-[1.75%] text-[#111111] font-semibold overflow-hidden whitespace-nowrap text-ellipsis">
                {row.description}
              </div>
              <div className="flex items-center justify-center px-[1%] text-[1.75%] text-[#111111] font-semibold overflow-hidden whitespace-nowrap text-ellipsis">
                {row.qty}
              </div>
              <div className="flex items-center justify-end px-[7%] text-[1.75%] text-[#111111] font-semibold">
                {row.unitPrice}
              </div>
              <div className="flex items-center justify-end pr-[8%] text-[1.75%] text-[#111111] font-semibold">
                {row.total}
              </div>
            </div>
          );
      ...
      <div className="absolute right-[4.5%] top-[47.35%] w-[32.5%] text-[1.8%]">
        {[
          ...
          <div
            key={idx}
            className="h-[3.1%] flex items-center justify-end pr-[7%] text-[#111111] font-semibold"
          >
            {value}
          </div>
        ))}
            ...
            <div className="mt-3 grid gap-2">
              {items.length === 0 ? (
                ...
                items.map((row, idx) => (
                  <div
                    key={row.id}
                    className="grid grid-cols-[28px_minmax(0,1fr)_64px_70px_76px_62px] sm:grid-cols-[32px_minmax(0,1fr)_70px_90px_90px_64px] gap-2 items-start border border-zinc-200 rounded-xl p-2      
              overflow-hidden"
                  >
