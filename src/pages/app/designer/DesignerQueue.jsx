...
export default function DesignerQueue() {
  ...
  async function load() {
    try {
      ...
      const [a, b, c] = await Promise.all([
        listDesignerJobsByStatus("DESIGN_ASSIGNED"),
        listDesignerJobsByStatus("DESIGN_PENDING"),
        listDesignerJobsByStatus("DESIGN_WAITING"),
      ]);
  ...
  async function markWaiting(jobId) {
    try {
      await designerWorkflow(jobId, "DESIGNER_SET_WAITING");
  ...
  async function start(jobId) {
    try {
      ...
      dialog.toast("Design started", "success");
      load();
    } catch (e) {
      dialog.toast(e?.response?.data?.message || "Failed", "error");
    }
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_348px]">
      <div className={workPageCardClass}>
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-2xl font-semibold text-primary">Design Queue</h2>
            ...
            <tbody>
              {slice.map((job) => (
                <tr
                  ...
                  className={cn(workRowClass, selected?.id === job.id && selectedRowClass)}
                >
                  ...
                  <td className={workTdClass}>
                    <div className="flex items-center gap-2 flex-nowrap">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); start(job.id); }} className={actionBtnClass("primary")}>Start Design</button>
                    </div>
                  </td>
                </tr>
              ))}
              {slice.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-sm font-semibold text-zinc-500">No queued jobs.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
      ...
      <JobDetailActionPanel selected={selected}>
        {selected ? (
          <>
            ...
            <button onClick={() => start(selected.id)} className={actionBtnClass("primary")}>Start Design</button>
          </>
        ) : null}
