...
export default function OperatorInProduction() {
  ...
  async function load() {
    try {
      setErr("");
      const all = (await listOperatorJobsByStatus("IN_PRODUCTION") || []).sort((x, y) => new Date(y.createdAt) - new Date(x.createdAt));
      setJobs(all);
  ...
  async function complete(jobId) {
    try {
      await operatorWorkflow(jobId, "OPERATOR_COMPLETE");
      dialog.toast("Production completed", "success");
      load();
    } catch (e) {
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
      <div className={workPageCardClass}>
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-2xl font-semibold text-primary">In Production</h2>
            ...
            <tbody>
              {slice.map((job) => (
                <tr key={job.id} onClick={() => setSelected(job)} className={cn(workRowClass, selected?.id === job.id && selectedRowClass)}>
                  ...
                  <td className={workTdClass}><button onClick={(e) => { e.stopPropagation(); complete(job.id); }} className={actionBtnClass("success", true)}>Production Completed</button></td>
                </tr>
              ))}
              {slice.length === 0 ? <tr><td colSpan={5} className="px-4 py-6 text-sm font-semibold text-zinc-500">No active production jobs.</td></tr> : null}
            </tbody>
          </table>
        </div>
      ...
      <JobDetailActionPanel selected={selected}>
        {selected ? <button onClick={() => complete(selected.id)} className={actionBtnClass("success", true)}>Production Completed</button> : null}
