...
export default function OperatorOverview() {
  ...
  const basePath = location.pathname.startsWith("/app/admin/operator") ? "/app/admin/operator" : "/app/operator";
  ...
  async function load() {
    try {
      setErr("");
      const a = await listOperatorJobsByStatus();
      setAll(a || []);
    } catch (e) {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: all.length,
      queue: ["PRODUCTION_READY", "PRODUCTION_PENDING", "PRODUCTION_WAITING"].reduce((a, s) => a + count(s), 0),
      inProd: count("IN_PRODUCTION"),
      done: count("PRODUCTION_DONE"),
    };
  }, [all]);

  const activeRows = useMemo(() => all.filter((j) => ["PRODUCTION_READY", "PRODUCTION_PENDING", "PRODUCTION_WAITING", "IN_PRODUCTION"].includes(j.status)), [all]);
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Card title="In Production" value={stats.inProd} sub="Active work" onClick={() => navigate(`${basePath}/in-production`)} />
        <Card title="Completed" value={stats.done} sub="Production done" onClick={() => navigate(`${basePath}/completed`)} />
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm transition-all duration-300 hover:shadow-md hover:border-primary/20">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-extrabold text-primary">Active Production Jobs</h2>
                ...
                <tr key={j.id} className="border-t border-zinc-200 hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="py-3 px-4 whitespace-nowrap">
                    <div className="flex items-center gap-2 flex-nowrap">
                      {["PRODUCTION_READY", "PRODUCTION_PENDING"].includes(j.status) && <button onClick={() => run(j.id, "OPERATOR_SET_WAITING", "Marked waiting")} className="px-3 py-2 rounded-xl bg-warning text-white font-semibold transition-all duration-300 hover:-translate-y-0.5 hover:shadow-sm">Mark Waiting</button>}
                      {["PRODUCTION_READY", "PRODUCTION_PENDING", "PRODUCTION_WAITING"].includes(j.status) && <button onClick={() => run(j.id, "OPERATOR_START", "Production started")} className="px-3 py-2 rounded-xl bg-primary text-white font-semibold transition-all duration-300 hover:-translate-y-0.5 hover:shadow-sm">Start Production</button>}
                      {j.status === "IN_PRODUCTION" && <button onClick={() => run(j.id, "OPERATOR_COMPLETE", "Production completed")} className="px-3 py-2 rounded-xl bg-success text-white font-semibold transition-all duration-300 hover:-translate-y-0.5 hover:shadow-sm">Complete</button>}
                    </div>
                  </td>
                </tr>
              ))}
              {slice.length === 0 && <tr><td colSpan={5} className="py-4 px-3 text-zinc-500">No active production jobs.</td></tr>}
            </tbody>
          </table>
        </div>
